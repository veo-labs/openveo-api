 ...
 * @param {Number} [width] The expected image width (in px) (default to 10)
 ...
 */
function generateThumbnail(imagePath, cachePath, width, quality, callback) {
  ...
  image.size(function(error, size) {
    ...
    .noProfile()
    .quality(quality)
    .resizeExact(width || 10, parseInt(size.height * width / size.width))
    .write(cachePath, callback);
  });
}
...
module.exports = function(imagesDirectory, cacheDirectory, styles, headers) {
  ...
  cacheDirectory = cacheDirectory || path.join(imagesDirectory, '.cache');
  headers = headers || {};
      ...
      readable.on('end', function() {
        ...
        fs.stat(imageCachePath, function(error, stats) {
          ...
          fileSystem.mkdir(path.dirname(imageCachePath), function(error) {
            ...
            generateThumbnail(fileSystemPath, imageCachePath, style.width, style.quality, function(error) {
              if (error) return next(error);
