 ...
 * belong to a particular user can be manipulated by this particular user, the super administrator and, if entity is
 * inside a group, by all users which have enough privileges on this group.
 *
    ...
    provider.getOne(
      ...
      function(error, entity) {
        ...
        if (self.isUserAuthorized(request.user, entity, ContentController.OPERATIONS.UPDATE)) {
          ...
          if ((!self.isUserOwner(entity, request.user) && !self.isUserAdmin(request.user)))
            delete data['user'];
      ...
      function(error, entities, pagination) {
        ...
        provider.remove(new ResourceFilter().in('id', entityIdsToRemove), function(error, total) {
          if (error) {
            ...
            process.logger.error('Some entities could not be removed',
...
ContentController.prototype.removeMetatadaFromFields = function(fields) {
  ...
  /**
   * Checks whether the given String does not contain "metadata".
   *
   * @param {String} text The text to test
   * @return {Boolean} true if text is clean and does not contain the chain "metadata", false otherwise
   */
  function isClean(text) {
    return text.indexOf('metadata') < 0;
  }

  if (fields.exclude) fields.exclude = fields.exclude.filter(isClean);
  if (fields.include) fields.include = fields.include.filter(isClean);
