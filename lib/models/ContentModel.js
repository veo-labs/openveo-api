 ...
 *     ['read-group-Jekrn20Rl', 'update-group-Jekrn20Rl', 'delete-group-YldO3Jie3', 'some-other-permission']
 *
 *     // Example of returned groups
 *     {
 *       'Jekrn20Rl': ['read', 'update'], // User only has read / update permissions on group 'Jekrn20Rl'
...
function getUserGroups(user) {
  ...
  if (user && user.permissions) {
    user.permissions.forEach(function(permission) {
      var reg = new RegExp('^(read|update|delete)-group-(.+)$');
 ...
 * @private
 * @param {String} operation The operation (read, update or delete)
...
ContentModel.READ_OPERATION = 'read';
...
ContentModel.prototype.isUserAuthorized = function(entity, operation) {
  if (this.isUserAdmin() ||
      !this.user ||
      entity.metadata.user === applicationStorage.getAnonymousUserId() ||
      entity.metadata.user === this.user.id
  ) {
    return true;
  }

  if (entity.metadata.groups) {
