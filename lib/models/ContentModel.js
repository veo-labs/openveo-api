 ...
 *
 *     // Use CustomContentModel
 *     var coreApi = openVeoApi.api.getCoreApi();
 *     var CustomEntityProvider = process.require('CustomEntityProvider.js');
 *
 *     var model = new CustomContentModel(
 *       {id: 42, permissions: [], groups: []},
 *       new CustomEntityProvider(coreApi.getDatabase())
 *     );
 *
 *     model.getOne(1, function(error) {
 *
 *       // Error if the user is not authorized to access the entity
 *
 *     });
 *
 * @class ContentModel
...
ContentModel.prototype.isUserAdmin = function() {
  return this.user && this.user.id === api.getCoreApi().getSuperAdminId();
};
...
ContentModel.prototype.isUserAuthorized = function(entity, operation) {
  if (this.isUserAdmin() ||
      ...
      !this.user ||
      (entity.metadata && entity.metadata.user === api.getCoreApi().getAnonymousUserId())
  ) {
    return true;
  }
...
ContentModel.prototype.add = function(data, callback) {
  data.metadata = {
    user: (this.user && this.user.id) || api.getCoreApi().getAnonymousUserId(),
    groups: data.groups || []
  };
  ...
  this.provider.getOne(id, null, function(error, entity) {
    if (!error) {
      if (self.isUserAuthorized(entity, ContentModel.UPDATE_OPERATION)) {
        ...
        if ((!self.isUserOwner(entity) && !self.isUserAdmin()))
...
ContentModel.prototype.addAccessFilter = function(filter) {
  if (!this.isUserAdmin() && this.user) {
    ...
    filter.$or.push({
      'metadata.user': {
        $in: [this.user.id, api.getCoreApi().getAnonymousUserId()]
      }
    });
