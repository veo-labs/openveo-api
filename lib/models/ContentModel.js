...
function ContentModel(user, provider) {
  EntityModel.call(this, user, provider);
}
 ...
 *
 * If no user is associated to the model, authorization is granted, same if the entity belongs to the anonymous
 * user.
 *
 ...
 */
ContentModel.prototype.isUserAuthorized = function(entity) {
  if (this.isUserAdmin() || !this.user || entity.metadata.user === applicationStorage.getAnonymousUserId())
    return true;

  return entity.metadata.user === this.user.id;
};
...
ContentModel.prototype.getOne = function(id, filter, callback) {
  ...
  this.provider.getOne(id, filter, function(error, entity) {
    if (!error && !self.isUserAuthorized(entity)) {
      var userId = self.user.id;
      callback(new Error('User "' + userId + '" doesn\'t have access to entity "' + id + '"'));
    } else
...
ContentModel.prototype.add = function(data, callback) {
  data.metadata = {user: (this.user && this.user.id) || applicationStorage.getAnonymousUserId()};
  ...
  this.provider.get({id: {$in: ids}}, function(error, entities) {
    if (!error) {
      ...
      for (var i = 0; i < entities.length; i++) {
        if (self.isUserAuthorized(entities[i]))
