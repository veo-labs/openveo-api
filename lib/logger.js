...
module.exports.add = function(name, conf) {
  if (!winston.loggers.loggers[name]) {
    ...
    // Remove Console transport, which log to the standard output
    if (process.env.NODE_ENV == 'production' || !conf || conf.console === false)
      this.get(name).remove(winston.transports.Console);
...
module.exports.get = function(name) {
  return winston.loggers.loggers[name];
};
